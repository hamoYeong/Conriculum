...
struct V1ContentValidatorTests {
    ...
    func brokenCatalogRelationAndMissingIdentityAreRejected() throws {
        ...
        let brokenManifest = V1ContentIdentityManifest(
    ...
    /// V1Chapter마다 다른 lesson 수를 허용하면서 현재 개수 안의 연속 order는 강제한다.
    @Test
    func nonContiguousOrderAndKnowledgeContextReportExactFields() throws {
        let content = try loadValidContent()
        var pages = content.chapter.pages
        let firstPage = pages[0]
        let brokenContext = V1PageKnowledgeContext(
    ...
    /// `chapter-02 → V1Chapter`, `values-and-types → KnowledgeCatalog`,
    /// `content-identity → V1ContentIdentityManifest` 순서로 JSON을 typed Domain 값으로 바꾼다.
    private func loadValidContent() throws -> ValidContent {
        let decoder = ContentResourceDecoder()
        return try ValidContent(
            chapter: decoder.decode(V1Chapter.self, from: .chapter02),
            catalog: decoder.decode(KnowledgeCatalog.self, from: .valuesAndTypes),
            identityManifest: decoder.decode(V1ContentIdentityManifest.self, from: .contentIdentity)
        )
    }
    ...
    private func validationError(
        ...
        identityManifest: V1ContentIdentityManifest
    ) -> V1ContentValidationError {
        do {
            try V1ContentValidator().validate(
                chapter: chapter,
                catalog: catalog,
            ...
            return V1ContentValidationError(issues: [])
    ...
    private func copyChapter(_ chapter: V1Chapter, pages: [V1LearningPage]) -> V1Chapter {
        V1Chapter(
            id: chapter.id,
    ...
    private func copyPage(
        ...
        knowledgeLinks: [V1LearningKnowledgeLink]? = nil
    ) -> V1LearningPage {
        V1LearningPage(
            id: page.id,
            kind: page.kind,
    ...
    private func copySection(
        ...
        content: V1LearningSectionContent
    ) -> V1LearningSection {
        V1LearningSection(
            id: section.id,
            order: section.order,
    ...
    private func brokenPromotionContent(
        _ content: V1LearningSectionContent
    ) -> V1LearningSectionContent {
