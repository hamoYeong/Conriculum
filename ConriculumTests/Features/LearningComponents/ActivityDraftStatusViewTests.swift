...
@MainActor
struct ActivityDraftStatusViewTests {
    @Test
    func everyVisibleSaveStateRendersAtStandardAndLargeText() throws {
        let timestamp = Date(timeIntervalSince1970: 1_700_000_000)
        let states: [(name: String, value: ActivityDraftSaveState)] = [
            ("pending", .pending),
            ("saving", .saving),
    ...
    private func render(
        saveState: ActivityDraftSaveState,
        ...
        let rootView = ActivityDraftStatusView(activity: activity)
