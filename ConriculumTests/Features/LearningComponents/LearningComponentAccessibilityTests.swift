...
@MainActor
struct LearningComponentAccessibilityTests {
    ...
    func everyInteractiveFixtureHasANonDragKeyboardPath() throws {
        ...
        var paths: [LearningSectionTag: KeyboardPath] = [:]
    ...
    func cardSortingCanAssignAValidCardWithoutDragging() throws {
        ...
        #expect(receivedFields == [
            ActivityResponseField(
                key: LearningActivityFieldKey.card(card.id),
                values: [group.id]
            )
        ])
    }
    ...
    func codeAssemblyPickersWriteEveryLineWithoutDragging() throws {
        ...
        let component = CodeAssemblyComponent(
            content: content,
            activity: activity
        )
        ...
        #expect(
            receivedFields.enumerated().allSatisfy { pair in
                ...
                return fields == [
                    ActivityResponseField(
                        key: LearningActivityFieldKey.codeLine(index),
                        values: [piece.id]
                    )
                ]
            }
    ...
    func everyVisibleSaveStateHasASpokenDescription() {
        let states: [ActivityDraftSaveState] = [
            .pending,
            .saving,
        ...
        #expect(ActivityDraftSaveState.idle.accessibilityDescription == nil)
    }

    private func interactiveSection(
        tag: LearningSectionTag
    ) throws -> LearningSection? {
        let chapter = try ContentResourceDecoder().decode(
            Chapter.self,
            from: .chapter02
        )
