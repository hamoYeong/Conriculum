...
struct KnowledgeSystemFeatureTests {
    ...
    @Test
    func revisitRoutesToTheLearningPage() async throws {
        let catalog = try BundledContentStore().loadKnowledgeCatalog()
        let references = catalog.concepts.flatMap { $0.revisitPages ?? [] }
    ...
    @Test
    func catalogSupportsCollectionSearchAndConceptComparison() async throws {
        let catalog = try BundledContentStore().loadKnowledgeCatalog()
...
private struct KnowledgeTestError: LocalizedError {
    var errorDescription: String? { "지식 체계 조회 실패" }
}
