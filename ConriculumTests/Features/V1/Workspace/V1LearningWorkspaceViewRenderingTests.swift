...
@MainActor
@Suite(.serialized)
struct V1LearningWorkspaceViewRenderingTests {
    @Test(arguments: [720.0, 1_100.0])
    func shellRendersAtNarrowAndWideWindowWidths(
        width: Double
    ) throws {
        let view = V1LearningWorkspaceView(
            store: Store(
    ...
    private func assertLayoutFits(
        state: V1LearningWorkspaceFeature.State,
        width: Double
    ) throws {
        let height = 720.0
        let view = V1LearningWorkspaceView(
    ...
    func resizingPreservesMainScrollViewAndSidebarPreference() async throws {
        ...
        state.sidebarMode = .visible
        state.isInspectorPresented = true
        let store = Store(initialState: state) { V1LearningWorkspaceFeature() }
        let hostingView = NSHostingView(rootView: V1LearningWorkspaceView(store: store))
    ...
    func panelsUseMaximumWidthsWhenOpenedAndReopened(
        ...
        let store = Store(initialState: state) { V1LearningWorkspaceFeature() }
        let host = NSHostingView(rootView: V1LearningWorkspaceView(store: store))
