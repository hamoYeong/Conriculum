...
@MainActor
struct V1PersonalRelationEditorFeatureTests {
    ...
    func createSavesAndReadsBackTheRelationWithItsEvidence() async {
        ...
        let store = TestStore(initialState: newRelationState()) {
            ...
            $0.v1PersonalKnowledgeClient.saveRelation = { relation in
        ...
        async
    {
        ...
        let store = TestStore(
            initialState: V1PersonalRelationEditorFeature.State(
                editing: original,
    ...
    func failureKeepsTheStoredRelationAndEveryDraftFieldForRetry() async {
        ...
        let store = TestStore(
            ...
            $0.date.now = timestamp
            $0.v1PersonalKnowledgeClient.saveRelation = { _ in
                throw NSError(
                    domain: "V1PersonalRelationEditorFeatureTests",
                    code: 1,
                    userInfo: [
        ...
        async
    {
        ...
        await store.send(.reasonChanged(""))
        await store.send(.saveButtonTapped) {
            $0.validationMessage = V1PersonalRelationValidationError
                .missingReason.errorDescription
        }
    }

    private func newRelationState() -> V1PersonalRelationEditorFeature.State {
        V1PersonalRelationEditorFeature.State(
            ...
            contract: V1KnowledgeContextSnapshot.RelationCreationContract(
                sourceConceptIDs: [
