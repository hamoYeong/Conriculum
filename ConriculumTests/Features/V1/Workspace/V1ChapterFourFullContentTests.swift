...
struct V1ChapterFourFullContentTests {
    ...
    func transferWithholdsGuidanceAndFinalLessonConnectsNonContiguousConditions() throws {
        let chapter = try V1BundledContentStore().loadChapter("chapter-04")
    ...
    func conditionResponseSurvivesAutosaveAndReloadThroughRealPersistence() async throws {
        ...
        let chapter = try await assembly.v1CurriculumClient.loadChapter("chapter-04")
        ...
        let fields = [V1ActivityResponseField(key: "response", values: ["둘 다 참일 때도 ||는 true다."])]
        ...
        var state = V1ChapterLearningFeature.State(chapterID: chapter.id, currentPageID: page.id)
    ...
    func everySectionRendersAtNarrowAndRegularWidths() throws {
        ...
        let names = V1KnowledgeConceptNames(catalog: try content.loadCatalog())
        for page in chapter.allPages {
            for section in page.sections {
                for width in [480.0, 760.0] {
                    let activity = section.activityID.map {
                        V1LearningActivityInput(activityID: $0, fields: []) { _, _ in }
                    }
                    let view = V1LearningSectionView(
