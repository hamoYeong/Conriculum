...
@MainActor
struct V1ChapterLearningFeatureTests {
    ...
    func validSavedPageAndActivityResponseAreResumedByIdentity() async throws {
        ...
        let restoredDraft = V1ChapterLearningFeature.ActivityDraft(
            responseID: response.id,
            activityID: savedActivityID,
            fields: response.fields
        )
        let olderResponse = V1ActivityResponse(
        ...
        let store = TestStore(
            ...
            $0.v1KnowledgeCatalogClient.loadCatalog = { knowledgeCatalog }
    ...
    func firstPageNavigatesBackToOverview() async throws {
        ...
        let expectedProgress = V1LearningProgress(
            chapterID: chapter.id,
            currentPageID: chapter.overview.id,
            completedPageIDs: [],
            updatedAt: timestamp
        ...
        let store = TestStore(initialState: state) {
            ...
            $0.v1LearningRecordClient.saveProgress = { progress in
    ...
    func draftAndTargetPageAreSavedBeforeStateMoves() async throws {
        ...
        let expectedResponse = V1ActivityResponse(
            id: draft.responseID,
            activityID: activityID,
            pageID: sourcePageID,
            fields: fields,
        ...
        let targetResponse = V1ActivityResponse(
        ...
        let targetDraft = V1ChapterLearningFeature.ActivityDraft(
...
private actor LearningRecordSaveRecorder {
    enum Event: Sendable {
        case response(V1ActivityResponse)
        case progress(V1LearningProgress)
    }
...
private actor RetryingResponseSaver {
    private var recordedResponses: [V1ActivityResponse] = []

    func save(_ response: V1ActivityResponse) throws {
        recordedResponses.append(response)
        if recordedResponses.count == 1 {
            throw NSError(
                domain: "V1ChapterLearningFeatureTests",
                code: 1,
                userInfo: [
    ...
    func responses() -> [V1ActivityResponse] {
        recordedResponses
    }
}
