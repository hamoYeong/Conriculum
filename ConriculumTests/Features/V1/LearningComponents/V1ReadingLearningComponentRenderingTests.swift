...
@MainActor
struct V1ReadingLearningComponentRenderingTests {
    ...
    private func component(
        ...
        case let .situation(payload):
            AnyView(V1SituationComponent(content: payload))
        case let .comparison(payload):
            AnyView(V1ComparisonComponent(content: payload))
        case let .definition(payload):
            AnyView(V1DefinitionComponent(content: payload))
        case let .decisionCriteria(payload):
            AnyView(V1DecisionCriteriaComponent(content: payload))
        case let .codeExplanation(payload):
            AnyView(V1CodeExplanationComponent(content: payload))
        case let .processGuide(payload):
            AnyView(V1ProcessGuideComponent(content: payload))
        default:
            AnyView(EmptyView())
        }
    }
