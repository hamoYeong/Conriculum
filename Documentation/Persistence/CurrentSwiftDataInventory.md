...
현재 제품인 V2는 `ConriculumCurrent` SwiftData 컨테이너를 사용한다. 이 컨테이너는 CloudKit을 사용하지 않는다.
...
이 정책은 이전 V2 저장 구현에서 현재 SwiftData 구현으로 안전하게 옮기면서, 현재 런타임의 쓰기 경로를 SwiftData 한 곳으로 고정한다.
