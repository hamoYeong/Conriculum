...
struct V1KnowledgeContextSnapshot: Equatable, Sendable {
...
enum V1KnowledgeContextSnapshotComposerError: LocalizedError, Equatable {
...
struct V1KnowledgeContextSnapshotComposer {
    func compose(
        chapter: V1Chapter,
    ...
    ) throws -> V1KnowledgeContextSnapshot {
        guard let page = chapter.page(id: pageID) else {
            throw V1KnowledgeContextSnapshotComposerError.missingPage(pageID)
        }
        ...
        return V1KnowledgeContextSnapshot(
            pageID: page.id,
    ...
    private func currentQuestion(in page: V1LearningPage) -> String {
    ...
    static func chapterConceptIDs(in chapter: V1Chapter) -> [KnowledgeConceptID] {
    ...
    private func item(
        ...
        usage: String?,
        nearbyReason: String?
    ) throws -> V1KnowledgeContextSnapshot.ConceptItem {
        guard let concept = conceptsByID[conceptID] else {
            throw V1KnowledgeContextSnapshotComposerError.missingConcept(
                conceptID
            )
        }
        return V1KnowledgeContextSnapshot.ConceptItem(
            concept: concept,
    ...
    private func relationCreationContract(
        in page: V1LearningPage
    ) -> V1KnowledgeContextSnapshot.RelationCreationContract? {
        for section in page.sections {
            ...
            return V1KnowledgeContextSnapshot.RelationCreationContract(
