...
struct GameDraft: Equatable, Sendable {
    var selectedOptionIDs: Set<String> = []
    var matches: [String: String] = [:]
}

@Reducer
struct LearningFeature {
    @ObservableState
    struct State: Equatable {
        ...
        var page: LessonPage?
        var progress: CourseProgress = .empty
        var gameDrafts: [String: GameDraft] = [:]
        ...
        var chapter: LearningChapter? {
            page.flatMap { manifest?.chapter(id: $0.chapterID) }
        }

        var orderedPages: [PageReference] {
    ...
    enum NavigationResponse: Equatable {
        case loaded(LessonPage, CourseProgress)
        case savedCompletion(CourseProgress)
        case failed(String)
    }
    ...
    var body: some Reducer<State, Action> {
        Reduce { state, action in
            ...
            case let .gameMatchChanged(activityID, pairID, rightPairID):
                ...
                var draft = state.gameDrafts[activityID]
                    ?? GameDraft(matches: state.progress.activityResponses[activityID]?.matches ?? [:])
    ...
    private func saveResponse(
        ...
        state.progress.activityResponses[activity.id] = GameResponse(
