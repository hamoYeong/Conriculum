...
struct StageOneGameComponent: View {
    ...
    init(
        page: LessonPage,
        responses: [String: GameResponse] = [:],
        drafts: [String: GameDraft] = [:],
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            if let wordSystem = block.wordSystem {
                WordSystemComponent(content: wordSystem)
            } else if let knowledgeUnlock = block.knowledgeUnlock {
                KnowledgeUnlockComponent(content: knowledgeUnlock)
...
struct WordSystemComponent: View {
    let content: WordSystem
...
struct KnowledgeUnlockComponent: View {
    let content: KnowledgeUnlock
...
private struct GameActivityView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            MarkdownContent(markdown: activity.promptMarkdown)
    ...
    private func optionCard(_ option: GameActivity.Option) -> some View {
    ...
    private func matchingTarget(_ pair: GameActivity.Pair) -> some View {
    ...
    private func feedback(_ response: GameResponse) -> some View {
