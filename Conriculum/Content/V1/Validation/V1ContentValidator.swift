...
struct V1ContentValidator: Sendable {
    ...
    func validate(
        ...
        catalogResource: String = "V1/KnowledgeCatalog/values-and-types.json",
        identityResource: String = "V1/ContentManifest/content-identity.json"
    ) throws {
        var issues: [V1ContentValidationIssue] = []
        ...
        if issues.isEmpty == false {
            throw V1ContentValidationError(issues: issues)
        }
    }

    // MARK: V1Chapter 내부 구조

    /// overview/lesson 구분, 연속 order, ID 중복, 필수 문맥과 완료 section을 검사한다.
    private func validatePageStructure(
        _ chapter: V1Chapter,
        resource: String,
        issues: inout [V1ContentValidationIssue]
    ) {
    ...
    /// V1Chapter/Page/Concept stable ID가 source manifest에도 등록되어 있는지 검사한다.
    private func validateIdentity(
        chapter: V1Chapter,
        catalog: KnowledgeCatalog,
        manifest: V1ContentIdentityManifest,
        resource: String,
        issues: inout [V1ContentValidationIssue]
    ) {
...
struct V1ContentValidationError: Error, Sendable, CustomStringConvertible {
    let issues: [V1ContentValidationIssue]
...
struct V1ContentValidationIssue: Equatable, Sendable, CustomStringConvertible {
    let resource: String
...
private extension V1LearningSectionContent {
